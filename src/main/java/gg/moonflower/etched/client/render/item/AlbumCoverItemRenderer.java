...
public class AlbumCoverItemRenderer extends BlockEntityWithoutLevelRenderer implements PreparableReloadListener {
    ...
    private static final ModelResourceLocation BLANK_ALBUM_COVER = new ModelResourceLocation(ResourceLocation.fromNamespaceAndPath(Etched.MOD_ID, FOLDER_NAME + "/blank"), "inventory");
    private static final ModelResourceLocation DEFAULT_ALBUM_COVER = new ModelResourceLocation(ResourceLocation.fromNamespaceAndPath(Etched.MOD_ID, FOLDER_NAME + "/default"), "inventory");
    private static final ResourceLocation ALBUM_COVER_OVERLAY = ResourceLocation.fromNamespaceAndPath(Etched.MOD_ID, "textures/item/album_cover_overlay.png");
...
//        ReloadListenerRegistry.register(PackType.CLIENT_RESOURCES, INSTANCE, ResourceLocation.fromNamespaceAndPath(Etched.MOD_ID, "builtin_album_cover"));
//        ClientNetworkEvent.DISCONNECT.register((controller, player, connection) -> INSTANCE.close());
    ...
    private static class DynamicModelData extends TextureAtlasSprite implements ModelData {
        ...
        private static final ResourceLocation ATLAS = ResourceLocation.fromNamespaceAndPath(Etched.MOD_ID, DigestUtils.md5Hex(UUID.randomUUID().toString()));
        private BakedModel model;

        private DynamicModelData(NativeImage image) {
            super(ATLAS, new SpriteContents(ResourceLocation.fromNamespaceAndPath(Etched.MOD_ID, DigestUtils.md5Hex(UUID.randomUUID().toString())), new FrameSize(image.getWidth(), image.getHeight()), image, AnimationMetadataSection.EMPTY, null), image.getWidth(), image.getHeight(), 0, 0);
        }
